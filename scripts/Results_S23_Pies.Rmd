...
```{r}
# get summary
  taxa_flow <- ddply(s23, .(taxa, tele_cat, sig_effect), summarize,
...
Make a new column for significant sig_effects; non-significant becomes a new level in this dataset.

```{r}
# new column of significant sig_effects; non-significant sig_effects renamed
  s23$sig_sig_effect <- ''
  s23$sig_sig_effect <- as.character(s23$significant)
  s23$sig_effect <- as.character(s23$sig_effect)
  s23$sig_sig_effect[is.na(s23$sig_sig_effect)] <- "Unknown Signif."
  s23$sig_sig_effect[s23$sig_sig_effect=='Not evaluated'] <- "Unknown Signif."
  s23$sig_sig_effect[s23$sig_sig_effect=='FALSE'] <- "Not Signif."
  s23$sig_sig_effect[s23$sig_sig_effect=='TRUE'] <- s23$sig_effect[s23$sig_sig_effect=='TRUE']
```
...
```{r}
# get summary
  taxa_flow_sig <- ddply(s23, .(taxa, tele_cat, sig_sig_effect), summarize,
                        # total count of entries     
                        count=length(taxa))
# save as csv
  write.csv(taxa_flow_sig,
            paste0(tab.dir,'synthesis_sig_effects_taxa_and_flow_significant.csv'),
            row.names = TRUE)
...
# sig_effects level change
  taxa_flow_sig$sig_sig_effect <- gsub("\\s*\\([^\\)]+\\)", '',
                          as.character(taxa_flow_sig$sig_sig_effect))
  taxa_flow_sig$sig_sig_effect <- factor(taxa_flow_sig$sig_sig_effect,
...
# change colors
  #sig_effect_cols <- c('#4EB265','#999999','#A50026','#FDB366','#9A709E')
  #sig_effect_cols <- c('#4EB265','#777777','#DC050C','#F6C141','#882E72')
  sig_sig_effect_cols <- c('#4EB265','#777777','#DC050C','#F6C141',
...
# Matrix of pie charts
  taxa_flow_fig <- ggplot(taxa_flow_sig) + 
                    aes(x='', y = count, fill = sig_sig_effect) +
                    geom_bar(position='fill',
                             stat='identity', width = 1, size = 0.01) +
                    # activate for vertical or horizontal version
                      facet_grid(taxa ~ tele_cat, switch='y') + #horizontal
...
# save image
  ggsave(taxa_flow_fig,
         filename=paste0(fig.dir,
                         # activate for vertical or horizontal version
                           #'synthesis_sig_effects_taxa_and_flow_significant_Vert.png'),
                           'synthesis_sig_effects_taxa_and_flow_significant_Horz.png'),
         #height=6.5,width=7,units='in',
         dpi=600)
...
Convert to long format, where metric is indicated and the significant sig_effect category is used instead.
...
# subset
  sig_metrics <- subset(s23,
                    select=c('paper_id','entry_id',
                            'taxa','tele_cat',
                            'sig_sig_effect', ##<--- THIS IS THE CHANGE HERE
                            'biodiv_cat_1sp',
                            'biodiv_cat_multsp',
                            'biodiv_cat_habitat'))
# melt
  sig_met_long <- melt(sig_metrics, id.vars=c('paper_id','entry_id',
                            'taxa','tele_cat','sig_sig_effect'))
...
Make a new table summarizing dataset by metric, coupling category and significant sig_effect

```{r}
# get summary
  met_flow_sig <- ddply(sig_met_long,
                       .(metric_type, metric, tele_cat, sig_sig_effect),
                       summarize,
                      # total count of entries     
                        count=length(metric))
...
# save as csv
  write.csv(met_flow_sig,
            paste0(tab.dir,
                   'synthesis_sig_effects_metric_and_flow_significant.csv'),
            row.names = FALSE)
...
```{r}
# sig_effects level change
  met_flow_sig$sig_sig_effect <- gsub("\\s*\\([^\\)]+\\)", '',
                          as.character(met_flow_sig$sig_sig_effect))
  met_flow_sig$sig_sig_effect <- factor(met_flow_sig$sig_sig_effect,
                              c('Positive','Neutral','Negative','Changed',
                               'Not Signif.',
                               'Unknown Signif.',
                               'Unclear'))
...
# save image
  ggsave(met_flow_sig_fig,
         filename=paste0(fig.dir,
         # activate for vertical or horizontal version
           #'synthesis_sig_effects_metric_and_flow_significant_Vert.png'),
           'synthesis_sig_effects_metric_and_flow_significant_Horz.png'),
         #height=6.5,width=7,units='in',
         dpi=600)
...
# save image
  ggsave(met_flow_sig_fig,
         filename=paste0(fig.dir,
         # activate for vertical or horizontal version
           #'synthesis_sig_effects_metric_type_and_flow_significant_Vert.png'),
           'synthesis_sig_effects_metric_type_and_flow_significant_Horz.png'),
