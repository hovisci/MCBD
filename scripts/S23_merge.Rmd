...
Determine Majority effect by paper and taxa  (NEED TO FINISH)

```{r}
# repeat taxa rows

taxID <- s23
taxID$taxID <- paste0(taxID$paper_id,'_',taxID$taxa)
nrow(taxID)
length(unique(taxID$paper_id))    # number of papers n=127
length(unique(taxID$taxID))       # number of paper:taxa n=132
length(unique(taxID$taxID)) - length(unique(taxID$paper_id))  # number of papers with multiple taxa (n=5)       

q <- as.data.frame(table(unique(taxID$taxID))) 
