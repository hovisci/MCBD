...
Note that n=131, some papers reported metrics for multiple taxa. Maj_eff was determined based on paper and taxa for these figures. 
...
# show levels
#  summary(tcRep$tc)
tc_tab <- as.data.frame(table(tcRep_maj$maj_eff))
...
```{r}
# Get continents, tc_cat, and maj_eff
ctr_tc_raw <- subset(tcRep_maj, select = c('paper_id', 'bd_continents', 'tc', 'maj_eff'))
...
```{r}
# sig_effects level change
  ctr_tc_raw$maj_eff <- factor( ctr_tc_raw$maj_eff,
                             c( "Beneficial","Harmful","Changed","Mixed","Not significant or unclear"))
# telecoupling level change
  ctr_tc_raw$tc <- factor(ctr_tc_raw$tc,
                                c("Energy Transfer","Investment","Knowledge Transfer",
